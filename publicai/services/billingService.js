(function () {
  function paymentProvider(providerId = "stripe") {
    const key = String(providerId || "stripe").toLowerCase();
    const adapters = {
      stripe: {
        createCheckout: (amountEur) => window.ZwimaStripeClient.createCheckout(amountEur),
        handleWebhook: (payload) => Promise.resolve({ ok: true, payload }),
        cancelSubscription: (subscriptionId) =>
          Promise.resolve({ ok: true, subscriptionId, provider: "stripe" }),
      },
      paypal: {
        createCheckout: async (amountEur) => ({
          mode: "mock",
          provider: "paypal",
          amountEur,
        ...
        cancelSubscription: async (subscriptionId) => ({ ok: true, subscriptionId, provider: "paypal" }),
      },
      mollie: {
        createCheckout: async (amountEur) => ({
          mode: "mock",
          provider: "mollie",
          amountEur,
          sessionId: `mollie_${Date.now()}`,
        }),
        handleWebhook: async () => ({ ok: true }),
        cancelSubscription: async (subscriptionId) => ({ ok: true, subscriptionId, provider: "mollie" }),
      },
      lemonsqueezy: {
        createCheckout: async (amountEur) => ({
          mode: "mock",
          ...
          amountEur,
          sessionId: `lemonsqueezy_${Date.now()}`,
        }),
        handleWebhook: async () => ({ ok: true }),
        cancelSubscription: async (subscriptionId) => ({ ok: true, subscriptionId, provider: "lemonsqueezy" }),
      },
    };
    return adapters[key] || adapters.stripe;
  }
  ...
  window.ZwimaBillingService = {
    ...
    createCheckout(amountEur, providerId = "stripe") {
      return paymentProvider(providerId).createCheckout(amountEur).then(async (session) => {
